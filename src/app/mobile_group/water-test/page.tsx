...
export default function WaterTestPage() {
    ...
    const getRiskLevel = () => {
        ...
        if (ph < 6.5 || ph > 8 || turbidity === 'cloudy') {
            return { level: 'medium', color: 'var(--warning)', label: 'Moderate Risk' };
        }
    ...
    async function handleSubmit(e: React.FormEvent) {
        ...
        try {
            await fetch('/api/reports', {
                method: 'POST',
                body: JSON.stringify({ type: 'water', ...report }),
        ...
        return (
            <div style={{
                ...
                <div className="card animate-slide-up" style={{ textAlign: 'center', maxWidth: 400 }}>
                    ...
                    {risk && (
                        <div className={`badge badge-${risk.level}`} style={{ marginBottom: 'var(--space-4)' }}>
                            {risk.label}
                        </div>
                    )}
                    ...
                    <div style={{ display: 'flex', gap: 'var(--space-3)', justifyContent: 'center' }}>
                        ...
                        <button onClick={() => setSubmitted(false)} className="btn btn-warning">
                            New Test
                        </button>
                    </div>
                </div>
        ...
        <div style={{
            ...
            <div style={{ maxWidth: 600, margin: '0 auto', padding: 'var(--space-4)' }}>
                <form onSubmit={handleSubmit}>
                    ...
                    {risk && (
                        <div
                            className={`alert alert-${risk.level === 'critical' ? 'critical' : risk.level === 'medium' ? 'warning' : 'success'}`}
